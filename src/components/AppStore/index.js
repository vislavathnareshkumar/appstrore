...
class AppStore extends Component {
  state = {
    ...
    filteredApps: appsList.filter(app => app.category === tabsList[0].tabId),
  }

  handleTabChange = tabId => {
    const filteredApps = appsList.filter(app => app.category === tabId)
    this.setState({activeTabId: tabId, filteredApps})
  }
  ...
  render() {
    const {activeTabId, filteredApps} = this.state
          ...
          <ul className="tabs-container">
            {tabsList.map(each => (
              <TabItem
                ...
                onClickTab={this.handleTabChange}
              />
            ))}
          </ul>
          <ul className="apps-container">
            {filteredApps.map(each => (
              <AppItem key={each.appId} app={each} />
            ))}
          </ul>
        </div>
      </div>
